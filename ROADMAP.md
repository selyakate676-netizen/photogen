...
1. Nano Banana создаёт нейтральный шаблон: композиция, тело, одежда, поза, фон, настроение.
2. Пользовательская identity появляется только после обучения LoRA.
3. Для дальних кадров сравниваются два способа применения identity к NB template: face-swap и LoRA img2img.
